...
/// Registry adapter: allows tools::add("tmux", version) to dispatch here
...
fn install_windows() -> Result<(), InstallError> {
    // There isn't a widely accepted official winget package for tmux.
    // Recommend using WSL (install via apt/dnf/etc.) or MSYS2/Chocolatey per environment.
    Err(InstallError::Prereq(
        "tmux installation on Windows is not automated. Install via WSL (apt/dnf/etc.) or a Unix layer like MSYS2.",
    ))
}
