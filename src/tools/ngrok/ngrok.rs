...
/// Ensure `ngrok` is installed. Ignore version hints; presence is sufficient.
...
/// Registry adapter: allows tools::add("ngrok", version) to dispatch here
...
fn install_linux() -> Result<(), InstallError> {
    if let Some(pm) = crate::tools::common::detect_linux_pm() {
        ...
        // Attempt to install via distro package manager. Some distros may not provide it.
        crate::tools::common::PkgOps::install(pm, "ngrok", crate::tools::common::default_use_sudo())
    } else {
...
fn install_windows() -> Result<(), InstallError> {
    ...
    // Official ngrok package on winget
    run("winget", &["install", "-e", "--id", "Ngrok.Ngrok"])?;
    Ok(())
}
