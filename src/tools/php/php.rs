...
/// Ensure `php` is available. We don't enforce a specific version; having
...
/// Registry adapter: allows tools::add("php", version) to dispatch here
...
fn install_linux() -> Result<(), InstallError> {
    if let Some(pm) = crate::tools::common::detect_linux_pm() {
        ...
        // Most distros package as `php` in repos
        crate::tools::common::PkgOps::install(pm, "php", crate::tools::common::default_use_sudo())
    } else {
...
fn install_windows() -> Result<(), InstallError> {
    ...
    // Use official PHP from the PHP Group if available in winget. Alternative is Chocolately, but we standardize on winget here.
    run("winget", &["install", "-e", "--id", "PHP.PHP"])?;
    Ok(())
}
