...
/// Ensure `gleam` is available. We don't enforce a specific version; having
/// the command present is sufficient.
...
/// Registry adapter: allows tools::add("gleam", version) to dispatch here
...
fn install_linux() -> Result<(), InstallError> {
    if let Some(pm) = crate::tools::common::detect_linux_pm() {
        ...
        crate::tools::common::PkgOps::install(pm, "gleam", crate::tools::common::default_use_sudo())
    } else {
...
fn install_windows() -> Result<(), InstallError> {
    ...
    // Best-effort: winget identifier for Gleam (subject to change)
    // If this fails in your environment, install manually from https://gleam.run
    run("winget", &["install", "-e", "--id", "Gleam.Gleam"])?;
    Ok(())
}
