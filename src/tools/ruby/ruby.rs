...
/// Ensure `ruby` is available. We don't enforce a specific version; having
...
/// Registry adapter: allows tools::add("ruby", version) to dispatch here
...
fn install_linux() -> Result<(), InstallError> {
    if let Some(pm) = crate::tools::common::detect_linux_pm() {
        ...
        crate::tools::common::PkgOps::install(pm, "ruby", crate::tools::common::default_use_sudo())
    } else {
...
fn install_windows() -> Result<(), InstallError> {
    ...
    // RubyInstaller from the RubyInstallerTeam via winget
    run(
        "winget",
        &["install", "-e", "--id", "RubyInstallerTeam.Ruby"],
    )?;
    Ok(())
}
