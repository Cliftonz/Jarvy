...
pub(crate) fn initialize() -> CliConfig {
    ...
    if !jarvy_dir.exists() {
        ...
        let config_content = fs::read_to_string(&config_file_path).expect("Unable to read config file");

        // Deserialize the config.toml file
        let config: CliConfig = toml::from_str(&config_content).expect("Unable to parse config file");
