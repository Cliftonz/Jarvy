...
|----------|-----------|-------|---------------|--------------|
| [`node-npm`](https://github.com/bearbinary/jarvy/tree/main/examples/node-npm) | `package-lock.json` exists, no other lockfiles | Node.js + npm | `npm ci` | `package.json`, `package-lock.json`, `.nvmrc` |
| [`node-pnpm`](https://github.com/bearbinary/jarvy/tree/main/examples/node-pnpm) | `pnpm-lock.yaml` exists | Node.js + pnpm | `pnpm install --frozen-lockfile` | `package.json`, `pnpm-lock.yaml`, `.nvmrc`, `.npmrc` |
| [`node-bun`](https://github.com/bearbinary/jarvy/tree/main/examples/node-bun) | `bun.lockb` or `bunfig.toml` exists | Bun runtime + node fallback | `bun install --frozen-lockfile` | `package.json`, `bun.lockb`, `bunfig.toml` |
| [`deno`](https://github.com/bearbinary/jarvy/tree/main/examples/deno) | `deno.json` or `deno.jsonc` exists | Deno bundled toolchain | n/a (deno cache) | `deno.json`, `deno.jsonc`, `deno.lock` |
| [`python-api`](https://github.com/bearbinary/jarvy/tree/main/examples/python-api) | `requirements.txt`, no `uv.lock` or `pyproject.toml` managed by poetry | Python + pip + venv | `pip install -r requirements.txt` | `requirements.txt`, `pyproject.toml` |
| [`python-uv`](https://github.com/bearbinary/jarvy/tree/main/examples/python-uv) | `uv.lock` exists, or `pyproject.toml` with `[tool.uv]` | Python + uv | `uv sync --frozen` | `pyproject.toml`, `uv.lock`, `.python-version` |
| [`go-api`](https://github.com/bearbinary/jarvy/tree/main/examples/go-api) | `go.mod` exists, HTTP service shape | Go + air + goose + golangci_lint | `go mod download` | `go.mod`, `go.sum`, `.golangci.yml` |
| [`rust-cli`](https://github.com/bearbinary/jarvy/tree/main/examples/rust-cli) | `Cargo.toml` with single `[package]`, no `[workspace]` | Rust single-crate | n/a | `Cargo.toml`, `Cargo.lock` |
| [`rust-workspace`](https://github.com/bearbinary/jarvy/tree/main/examples/rust-workspace) | `Cargo.toml` with `[workspace]` | Rust multi-crate + nextest + deny + machete | `cargo fetch` | `Cargo.toml`, `Cargo.lock`, `rust-toolchain.toml` |
| [`ruby-rails`](https://github.com/bearbinary/jarvy/tree/main/examples/ruby-rails) | `Gemfile` + `config/application.rb` | Ruby on Rails + Postgres + Redis | `bundle install` | `Gemfile`, `Gemfile.lock`, `.ruby-version`, `.tool-versions` |
| [`java-spring`](https://github.com/bearbinary/jarvy/tree/main/examples/java-spring) | `pom.xml` with `spring-boot` parent, or Spring Boot Gradle plugin | Java + OpenJDK + Maven/Gradle + Spring Boot | `mvn dependency:go-offline` or `gradle dependencies` | `pom.xml`, `build.gradle`, `gradle.properties`, `.java-version` |
| [`react-app`](https://github.com/bearbinary/jarvy/tree/main/examples/react-app) | React + Vite project | Frontend (React/Vite) | depends on lockfile | varies |
| [`fullstack`](https://github.com/bearbinary/jarvy/tree/main/examples/fullstack) | Mixed frontend+backend repo | Full-stack monorepo | varies | varies |
| [`k8s-platform`](https://github.com/bearbinary/jarvy/tree/main/examples/k8s-platform) | Platform engineering repo (terraform, helm) | DevOps tooling stack | n/a | varies |
